...
const Product = {
  ...
  async remove(id) {
    try {
      // Check if product has any reviews and delete them
      ...
      if (orderItems.length > 0) {
        // Instead of preventing deletion, we'll implement soft delete
        // Mark product as deleted but keep it in database for order history
        const deletedProduct = await prisma.products.update({
          where: { id: Number(id) },
          data: { 
            name: `[DELETED] ${(await this.getById(id)).name}`,
            stock: 0,
            // We'll add a deleted flag in a future migration, for now just mark in name
          }
        });
        return { soft_deleted: true, product: deletedProduct };
      }
