...
export function ImageUpload({ 
  ...
  onImageUploaded, 
  currentImage = null, 
  uploadType = "product", // "product" | "banner" | "hero" | "homepage" | "collection"
  className = "" 
}) {
  const { token } = useAuth();
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      ...
      if (uploadType === 'product') {
        ...
        endpoint = '/admin/upload/collection-image';
      } else {
    ...
    <div className={`space-y-2 ${className}`}>
      ...
      <div className="flex flex-col space-y-2">
        {previewUrl ? (
          ...
          <div className="w-full h-48 border-2 border-dashed border-border rounded-md flex items-center justify-center">
